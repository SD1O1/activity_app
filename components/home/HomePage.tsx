...
export default function HomePage() {
  ...
  return (
    <main className="w-full px-4 pb-8 md:mx-auto md:max-w-[768px] lg:max-w-[1100px] xl:max-w-[1280px]">
    <Header centerSlot={<span />} />

    <section className="pt-6 lg:mt-12">
      <div className="max-w-3xl space-y-3">
        <h1 className="text-2xl font-semibold tracking-tight text-gray-900 md:text-3xl lg:text-4xl">Find your next adventure</h1>
        <button onClick={() => setOpenSearch(true)} className="h-11 w-full rounded-lg border border-transparent bg-gray-100 px-4 text-left text-sm text-gray-500 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400 md:h-12 md:px-5 md:text-base">
    ...
    <section className="pt-6 lg:mt-12">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {activities.map((activity) => {
          ...
          return (
            <button key={activity.id} onClick={() => router.push(`/activity/${activity.id}`)} className="flex h-full min-h-[200px] w-full flex-col rounded-2xl border border-gray-100 bg-white p-5 text-left shadow-sm transition-all duration-200 hover:shadow-md lg:p-6">
              <div className="flex items-center justify-between text-sm text-gray-500">
                <span>{activity.host?.name || "Host"}</span>
                <span>{activity.type === "group" ? "GROUP" : "1-ON-1"}</span>
              </div>
              <h3 className="mt-10 max-w-2xl text-base font-semibold leading-tight text-gray-900">{activity.title}</h3>
